...
export default connect(state => {
  ...
  if (!!startAccount && !!endAccount && !!startPeriod && !!endPeriod) {
    const accounts = state.accounts.reduce((map, item) => map.set(item.ACCOUNT, item.LABEL), new Map());
    const entries = state.journalEntries
      .filter(entry =>
        !!accounts.get(entry.ACCOUNT)
      ...
      .reduce((map, entry) => {
        let [debit, credit] = map.get(entry.ACCOUNT) || [0, 0];
        return map.set(entry.ACCOUNT, [debit + entry.DEBIT, credit + entry.CREDIT]);
      }, new Map());

    balance = Array.from(entries.keys()).map((key) => {
      const [DEBIT, CREDIT] = entries.get(key);
      return {
        ACCOUNT: key,
        DEBIT,
        CREDIT,
        BALANCE: DEBIT - CREDIT, 
        DESCRIPTION: accounts.get(key),
      };
    });
  }
